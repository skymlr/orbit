...
struct ThirdPartyCreditsTests {
    ...
    private func loadPackageResolved(filePath: StaticString = #filePath) throws -> ResolvedPackageFile {
        ...
        let candidateDirectories = sequence(first: fileURL.deletingLastPathComponent()) { directory in
            let parent = directory.deletingLastPathComponent()
            return parent == directory ? nil : parent
        }

        guard let resolvedURL = candidateDirectories
            .map({ $0.appendingPathComponent("Package.resolved") })
